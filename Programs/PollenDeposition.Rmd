...
data_2025 <- read.csv("Raw Data/Data/Ovule_Count_2025.csv")
meta_2025 <- read.csv("Raw Data/Data/Ovule_Count_2025_meta.csv", header = TRUE)
...
```{r}
# Stigma length
q1_stig.m <- lmer(Stigma_Length ~ Sex + (1|Population) + (1|Population:Plant.ID), data = Ovule_data)
...
(stig_plot <- ggplot(Ovule_data, aes(x = Sex, y = Stigma_Length, fill = Sex)) +
...
### All together
```{r}
qA_ovule_all.m <- glmer.nb(Ovule_Number_1 ~ Sex * Stigma_Length + (1|Population) + (1|Population:Plant.ID), data = stigma_all, verbose = TRUE)
...
q1_ovule_binom.m <- glmer(female_func ~ Sex * Stigma_Length + (1|Population) + (1|Population:Plant.ID), data = Ovule_data, family = "binomial")

summary(q1_ovule_binom.m)
Anova(q1_ovule_binom.m)

RE_ovule_binom.m <- glmer(female_func ~ Sex * Stigma_Length + (1|Population:Plant.ID), data = Ovule_data, family = "binomial")
anova(q1_ovule_binom.m, RE_ovule_binom.m)


emmeans(q1_ovule_binom.m, specs = pairwise ~ Population, type = "response")
...
(ov_plot1 <- ggplot(Ovule_data, aes(x = Stigma_Length, y = female_func)) +  
  geom_jitter(height = 0.1, color = "black") +
  geom_smooth(method = "glm", se = T, 
...
q1_ovule_num.m <- glmer.nb(Ovule_Number_1 ~ Sex * Stigma_Length + (1|Population) + (1|Population:Plant.ID), data = ovule1, verbose = TRUE)
...
(ov_plot3 <- ggplot(ovule1, aes(x = Stigma_Length, y = Ovule_Number_1)) +  
  geom_jitter(height = 0.1, color = "black") +
  geom_smooth(method = "glm", se = T, 
...
```{r}
qA_pollen_all_SL.m <- glmer.nb(Pollen_Number_1 ~ (Stigma_Length + true_HermCount + true_FemCount) * Sex + Population + (1|Plant.ID), data = stigma_all, verbose = TRUE)
...
RE_pollen_all_SL.m <- glm(Pollen_Number_1 ~ (Stigma_Length + true_HermCount + true_FemCount) * Sex + Population, data = stigma_all, family = "poisson")
...
```{r}
q2_pollen_depo_SL.m <- glmer(pollen_depo ~ (Stigma_Length + true_HermCount + true_FemCount) * Sex + Population + (1|Plant.ID), data = stigma_all, family = "binomial")
...
RE_pollen_depo_SL.m <- glm(pollen_depo ~ (Stigma_Length + true_HermCount + true_FemCount) * Sex + Population, data = stigma_all, family = "binomial")
...
# Combine figure for stigma length and micro sex ratio
...
q2_pollen_num_SL.m <- glmer.nb(Pollen_Number_1 ~ (Stigma_Length + true_HermCount + true_FemCount) * Sex + Population + (1|Plant.ID), data = pollen1, verbose = TRUE)
...
RE_pollen_num_SL.m <- glm(Pollen_Number_1 ~ (Stigma_Length + true_HermCount + true_FemCount) * Sex + Population, data = pollen1, family = "poisson")
anova(q2_pollen_num_SL.m, RE_pollen_num_SL.m)

(pollen_stig_plot2 <- ggplot(pollen1, aes(x = Stigma_Length, y = Pollen_Number_1, group = Sex, color = Sex)) +
...
```{r}
(pollenDepo_plot <- ggarrange(pollen_stig_plot1, pollen_stig_plot2, nrow = 1, ncol = 2, labels = c("A", "B"), widths = c(0.45,0.55)))

ggsave(pollenDepo_plot, file = "./Figures/Pollen_Depo.png", dpi = 300)
```
