...
```{r}
FloralTrait_Panel <- ggarrange(ggarrange(stig_plot, ov_plot1, ncol = 2, nrow = 1, labels = c("A", "B")), 
...
```{r}
qA_pollen_all_SL.m <- glmer.nb(Pollen_Number_1 ~ (Stigma_Length + true_HermCount + density) * Sex * Population + (1|Plant.ID), data = stigma_all, verbose = TRUE)
...
```{r}
q2_pollen_depo_SL.m <- glmer(pollen_depo ~ (Stigma_Length + true_HermCount + density) * Sex * Population + (1|Plant.ID), data = stigma_all, family = "binomial")
...
RE_pollen_depo_SL.m <- glm(pollen_depo ~ (Stigma_Length + true_HermCount + density) * Sex * Population, data = stigma_all, family = "binomial")
